...
// ConsistentIndexer is an interface that wraps the Get/Set/Save method
// for consistentIndex.
...
type consistentIndex struct {
	...
	// to avoid a repetitive allocation in saveInde.
	bytesBuf8 []byte

	mutex sync.Mutex
}

// NewConsistentIndex return a new backend consistent indexer
func NewConsistentIndex(tx backend.BatchTx) ConsistentIndexer {
	return &consistentIndex{
		tx:        tx,
...
// NewFakeConsistentIndex return a fake consistentIndexer for test usage
