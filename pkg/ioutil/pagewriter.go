...
// PageWriter implements the io.Writer interface so that writes
// will either be in page chunks or from flushing.
type PageWriter struct {
	...
	pageBytes int
	// bufferedByte counts the number of bytes pending for write in the buffer
	bufferedBytes int
	// buf holds the write buffer
	buf []byte
	// bufWatermarkBytes is the number of bytes the buffer can hold before it
	// needs to be flushed. It is less than len(buf) so there is space for slack
	// writes to bring the writer to page alignment.
...
func (pw *PageWriter) Write(p []byte) (n int, err error) {
	// if buffer still enough to hold the write
