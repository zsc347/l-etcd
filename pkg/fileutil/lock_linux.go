...
// An alternative is lockf() which works on  NFS but taht call lets a process lock
// the same file twice. Instead, use Linux's non-standard open file
...
// LockFile blocking api for lock file
func LockFile(path string, flag int, perm os.FileMode) (*LockedFile, error) {
	return linuxLockFile(path, flag, perm)
}

// TryLockFile no blocking api for lock file
