...
// Exist returns true if there are many files in a given directory.
...
// searchIndex returns the last array index of names whose raft index
// section is equal to or smaller than the given index.
...
func searchIndex(lg *zap.Logger, names []string, index uint64) (int, bool) {
	for i := len(names) - 1; i >= 0; i-- {
		...
		if err != nil {
			lg.Panic("failed to parse WAL file name", zap.String("path", name),
				zap.Error(err))
		}
		if index >= curIndex {
			return i, true
...
func isValidSeq(lg *zap.Logger, names []string) bool {
	...
	for _, name := range names {
		...
		if err != nil {
			lg.Panic("failed to parse WAL file name",
				zap.String("path", name), zap.Error(err))
		}
