...
// SetUnsafeNoFsync set wal no need call fsync
...
func (w *WAL) cleanupWAL(lg *zap.Logger) {
	...
	brokenDirName := fmt.Sprintf("%s.broken.%v",
		w.dir,
...
func (w *WAL) renameWAL(tmpdirpath string) (*WAL, error) {
	...
	// it's possible the process will fork to spawn a proces while this is
	// happening. the fds are set up as close-on-exec by the Go runtime,
	// but there is a window between the fork and the exec where another process
	// holds the lock.
...
func (w *WAL) renameWALUnlock(tmpdirpath string) (*WAL, error) {
	// rename of directory with locked files doesn't work on window/cifs;
	...
	// reopen and relock
	_, oerr := Open(w.lg, w.dir, walpb.Snapshot{})
	if oerr != nil {
		return nil, oerr
	}
...
// If opened in write mode, it must read out all records util EOF. Or an error
// will be returned.
...
func (w *WAL) ReadAll() (
	...
	for err = decoder.decode(rec); err == nil; err = decoder.decode(rec) {
		...
		case entryType:
			...
			if e.Index > w.start.Index {
				// prevent "panic: runtime error:
				// slice bounds out of range [:13038096702221461992] with capacity 0"
	...
	case nil:
		...
		// The last record may be a partial written one, so
...
// ValidSnapshotEntries returns all the valid snapshot entries in the wal logs in
// the given directory.
// Snapshot entries are valid if their index is less than or equal to the most recent
// committed hardstate.
func ValidSnapshotEntries(lg *zap.Logger, walDir string) ([]walpb.Snapshot, error) {
	...
	for err = decoder.decode(rec); err == nil; err = decoder.decode(rec) {
		...
		case crcType:
			crc := decoder.crc.Sum32()
			// current crc of decoder must match the crc of the record.
			// do no need to match 9 crc, since the decoder is a new one at this case.
			if crc != 0 && rec.Validate(crc) != nil {
...
func Verify(lg *zap.Logger, walDir string, snap walpb.Snapshot) error {
	...
	for err = decoder.decode(rec); err == nil; err = decoder.decode(rec) {
		...
		// We ignore all entry and state type records as these are not
		// necessary for validating the WAL contents
		case entryType:
		case stateType:
		default:
	...
	// We do not have to read out all the WAL entries as the decoder is
	// opened in read mode.
	if err != io.EOF && err != io.ErrUnexpectedEOF {
		return err
	}
...
// Then cut atomically rename temp wal fiel to a wal file.
func (w *WAL) cut() error {
	...
	// aotmiclly move temp wal file to wal file
	if err = w.sync(); err != nil {
		return err
	}
	...
	if newTail, err = fileutil.LockFile(
		fpath,
		os.O_WRONLY,
...
func (w *WAL) ReleaseLockTo(index uint64) error {
	...
	// if no lock index is greater thant he release index, we can
	// release lock up to the last one (excluing).
	if !found {
...
// Save save raft state and entries
...
func (w *WAL) seq() uint64 {
	...
	if err != nil {
		w.lg.Fatal("failed to parse WAL name",
