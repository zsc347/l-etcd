...
func (ti *treeIndex) Get(key []byte, atRev int64) (
	modified, created revision,
	ver int64,
	err error) {
	keyi := &keyIndex{key: key}
	ti.RLock()
...
func (ti *treeIndex) Revisions(key, end []byte,
	atRev int64,
	limit int) (revs []revision) {
	if end == nil {
...
func (ti *treeIndex) Range(key, end []byte, atRev int64) (
	keys [][]byte,
	revs []revision) {
	if end == nil {
		rev, _, _, err := ti.Get(key, atRev)
...
// at or after the given rev. The returned slice is stored in the order
// of revision
...
func (ti *treeIndex) Compact(rev int64) map[revision]struct{} {
	...
	clone.Ascend(func(item btree.Item) bool {
		...
		// compaction is going on or revision aded to empty before deletion
		ti.Lock()
