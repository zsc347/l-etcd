...
// Tombstone on a key appends an tombstone version at the end of
// the current generation and creates a new empty generation.
...
// get gets the modified, created revision and version of the key that
// satisfies the given atRev.
// Rev must be higher than or equal to the given atRev.
func (ki *keyIndex) get(lg *zap.Logger, atRev int64) (
	modified,
	created revision,
	ver int64,
	err error) {
...
// Only the revision with the largest sub revision will be returned if multiple revisions
// have the same main revision.
func (ki *keyIndex) since(lg *zap.Logger, rev int64) []revision {
	if ki.isEmpty() {
		lg.Panic("`since` got an unexpected empty keyIndex",
			zap.String("key", string(ki.key)))
	}
	...
	for ; gi < len(ki.generations); gi++ {
		for _, r := range ki.generations[gi].revs {
			...
			if r.main == last {
				// replace the revision with a new one that has higher sub balue.
...
// revision than the give atRev except the largest one (If the largest one is
// a tombstone, it will be kept).
// If a generation becomes empty during compaction, it will be removed.
func (ki *keyIndex) compact(lg *zap.Logger,
	atRev int64,
...
func (ki *keyIndex) doCompact(atRev int64, available map[revision]struct{}) (
	genIdx int, revIndex int) {
...
func (ki *keyIndex) findGeneration(rev int64) *generation {
	lastGeneration := len(ki.generations) - 1
	cg := lastGeneration
...
// walk returns the position at where is stopped. If it stopped after finishing
// walking, -1 will be returned.
func (g *generation) walk(f func(rev revision) bool) int {
	l := len(g.revs)
	for i := range g.revs {
		ok := f(g.revs[l-i-1]) /* reverse order */
		if !ok {
			return l - i - 1
		}
	}
